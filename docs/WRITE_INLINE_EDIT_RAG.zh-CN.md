...
- `src/main/ipc/app-ipc-schemas.ts`、`src/preload/index.ts`、`src/shared/sns-gui-api.ts`：复用 `write:inline-completion` IPC，通过 `mode: "edit"`、`editCandidate` 和 recent edits 承载 inline edit。
