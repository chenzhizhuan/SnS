...
- `src/main/ipc/app-ipc-schemas.ts`, `src/preload/index.ts`, `src/shared/sns-gui-api.ts`: the existing `write:inline-completion` IPC accepts `mode: "edit"` plus `editCandidate` and recent edits.
