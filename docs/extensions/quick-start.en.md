...
Open Kun, choose **Hello** directly from Code mode's `+` tool menu, and Kun opens it as an independent right-workspace tab. Rendering its icon and title does not activate Node code. Opening the View triggers `onView:hello` and creates an identity-bound View Session.
